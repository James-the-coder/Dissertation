...
from normaliser import RunningMeanStd
import os 
import numpy as np
import pandas as pd
...
class SAC_Agent():  
    def __init__(self, state_dim, goal_dim, action_dim, max_action, gamma=0.98, tau=0.05, alpha=0.2, alpha_fixed=True, learn_rate=3e-4, useRND=False, useICM=False, im_reward_scale=0.1):
        self.actor = Actor(state_dim, goal_dim, action_dim, max_action, half_cheetah=True).to(device)
        self.critic = Critic(state_dim, goal_dim, action_dim, half_cheetah=True).to(device)
...
if __name__ == "__main__":
    ...
    _, episodes, sac_settings, useRND, useICM, im_reward_scale = config_settings
    ...
    replay_buffer = HERBuffer(buffer_size=buffer_size, k_future=0)
    ...
    warmup_steps = 1000
    total_warmup_steps = warmup_steps * 1000
    obs, _ = env.reset()
